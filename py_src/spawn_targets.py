...
class TargetManager:
    ...
    def _spawn_single_model(
        self, model: TargetModel, spawn_name: str, x: float, y: float, z: float = 0.2
    ):
    ...
    def respawn_targets(self, dropzone_gps_boundary, home_lat, home_lon):
        ...
        models_to_spawn = random.sample(self.canopy_targets, 1) + random.sample(
            self.human_targets, 1
        )
        if len(self.noise) >= 2:
        ...
        for i, model in enumerate(models_to_spawn):
            ...
            while attempts < max_attempts:
                ...
                if not any(
                    np.linalg.norm(candidate_pos - p) < 2 * self.target_radius
            ...
            if target_pos is None:
                print(
                    f"Error: Could not find valid location for {model.name}. Skipping."
                )
                continue
